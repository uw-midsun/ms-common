...
static bool prv_raise_event(uint16_t i) {
  Event e = {
    .id = i,
    .data = i * 100
  };
...
void test_event_queue_raise(void) {
  ...
  for (int i = EVENT_QUEUE_SIZE; i > 0; i--) {
    TEST_ASSERT_TRUE(prv_raise_event(i));
  }

  // Attempt to insert an element when full
  TEST_ASSERT_FALSE(prv_raise_event(0));
  ...
  TEST_ASSERT_TRUE(prv_raise_event(5));
  TEST_ASSERT_TRUE(event_process(&e));

  TEST_ASSERT_EQUAL(5, e.id);
  TEST_ASSERT_EQUAL(5 * 100, e.data);

  TEST_ASSERT_FALSE(event_process(&e));
}
